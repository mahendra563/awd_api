...
class WorkEntryController extends ManagerController
{
    public function actions()
    {
        return [ 
            ...
            'get' => [
                'class' => Get::className(),
                'modelClass'=> WorkEntry::className(),                                  
                'with' =>["project","cost","alottedBy","alottedTo","userType","taskNature"],
    ...
    public function __construct($id, $module, $config = []) {
        ...
        // Event::on(WorkEntry::class,WorkEntry::EVENT_BEFORE_VALIDATE, [$this,"WorkEntry_Validate"]);
    ...
    public function WorkFile_beforeValidate(Event $e){
        ...
        public function WorkEntry_beforeValidate(Event $e){
            ...
            $emp  = TempData::i()->get("loggedInUser");
            $empm  = TempData::i()->get("project_managers");
            //  $proj_id = \Yii::$app->request->get("proj_id",0);
            //   $model = $this->_getWorkEntry($emp_id);
            if( $e->sender->isNewRecord){
                  //   $meps = projectManager::findone(\Yii::$app->request->post("join_id"));
                  $e->sender->w_alotted_by = $emp->emp_id;
                //   $e->sender->w_user_type = $empm->join_id;
